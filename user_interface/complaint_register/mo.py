        ...
        if latest_photo_path and latest_photo_path != previous_photo_path:
            ...
            if pull_photo(device_id, latest_photo_path, file_name):
                ...
                if latitude and longitude:
                    ...
                    complaint_id = str(uuid.uuid4())
                    email_sent = send_confirmation_email(email, complaint_id, latitude, longitude, file_name)
...
def send_email():
    ...
    email_sent = send_confirmation_email(email, complaint_id, latitude, longitude, image_path)
    if email_sent:
...
def get_previous_photo(device_id):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            previous_photo_path = result.stdout.splitlines()[0].strip()
            print(f"Previous photo path: {previous_photo_path}")  # Debugging line
            return previous_photo_path
        else:
            print("No photos found in the Camera directory.")
            return None
    except subprocess.CalledProcessError as e:
        print(f"Error retrieving photo from device: {e}")
        return None
...
def get_latest_photo(device_id):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            latest_photo_path = result.stdout.splitlines()[0].strip()
            print(f"Latest photo path: {latest_photo_path}")  # Debugging line
...
# Function to pull the photo from the device
def pull_photo(device_id, latest_photo_path, file_name):
    """Pull the latest photo from the device to the local machine."""
    try:
        pull_photo_command = f"adb -s {device_id} pull {latest_photo_path} ./{file_name}"
        subprocess.run(pull_photo_command, shell=True, check=True)
        print(f"Photo pulled from device and saved as {file_name}")
        return True
    except subprocess.CalledProcessError as e:
        print(f"Error pulling photo from device: {e}")
        return False
